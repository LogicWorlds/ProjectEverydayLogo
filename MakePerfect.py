...
def generateIcon(drawModeRandom=True, drawMode=2, drawTextMode=2, lw="LW", sendToDiscord=True, debug=False):
    ...
    while 1:
        if len(os.listdir(dir + "/props/bg/")) == 0:
            ...
            rand_imagebg = dir + "/props/bg/" + \
                random.choice(os.listdir(dir + "/props/bg/"))
            print(rand_imagebg)

        if drawMode == 2:
            rand_imagebg = io.BytesIO(requests.get(
    ...
    if drawTextMode == 1:
        rand_imagetext = dir + "/props/text/" + \
            random.choice(os.listdir(dir + "/props/text/"))
    ...
    if drawTextMode == 2:
        ...
        if bright_bg > 0.8:
            ...
            t_back_color = "#555"
        draw.text(((W - w) / 2 + 5, (H - h) / 2 + 5),
                  lw, fill=t_back_color, font=myFont)
